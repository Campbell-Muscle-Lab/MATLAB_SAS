...
addOptional(p,'y_main_label_offset',0.2);
...
addOptional(p,'temp_excel_file_string','temp.xlsx');
...
% First load the original data as a table
if (isempty(p.data_table))
    
    % If the table is not there, load it from excel
    p.data_table = readtable(p.excel_file_string, ...
                    'Sheet', p.excel_sheet);
end

% Now reduce the table to the data we actually need
[od, jd] = extract_two_way_data( ...
                   'table', p.data_table, ...
                   'parameter_string', p.test_variable, ...
                   'factor_1', p.factor_1, ...
                   'factor_1_strings', p.factor_1_strings, ...
                   'factor_2', p.factor_2, ...
                   'factor_2_strings', p.factor_2_strings, ...
                   'grouping_string', p.grouping);
               
% Now write that table to a temp file
try
    delete(p.temp_excel_file_string);
end
writetable(od, p.temp_excel_file_string);

% Now run the stats test
if (~isempty(p.grouping))
    stats = sas_two_way_anova_with_grouping( ...
                'excel_file_string',p.temp_excel_file_string, ...
...
jd_out = jitter_plot( ...
    ...
    'y_main_label_offset', p.y_main_label_offset, ...
